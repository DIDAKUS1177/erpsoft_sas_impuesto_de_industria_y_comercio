...
/*		$query = "SELECT SUM(round(cu.cuco_Valor)) as valor
		FROM fac_cuentascontables as cu WHERE cu.cuco_IdCuentaContable = 1 and
			cu.cuco_IdCierre = $idDocumento  and cu.cuco_Observacion like '%Abono/Pago Factura a Credito%';";
*/
		$query = "(SELECT SUM(round(cu.cuco_Valor)) as valor
		FROM fac_cuentascontables as cu WHERE cu.cuco_IdCuentaContable = 1 and
			cu.cuco_IdCierre = $idDocumento  and cu.cuco_Observacion like '%Abono/Pago Factura a Credito%')
            UNION ALL
            (SELECT CONCAT('-', SUM(round(cu.cuco_Valor))) as valor
		FROM fac_cuentascontables as cu WHERE cu.cuco_IdCuentaContable = 1 and
			cu.cuco_IdCierre = $idDocumento  and cu.cuco_Observacion like '%Reintegro de Anulación - Abono Cuenta por Cobrar%');";
		...
		if($rowDocuMovimienEfectivoTotal != NULL){
			$totalAbonos = $rowDocuMovimienEfectivoTotal[0]['valor']+$rowDocuMovimienEfectivoTotal[1]['valor'];
		}else{
			$totalAbonos = 0;
		}
	...
	if( isset($rowDocuClien[1])){
		...
		FROM fac_cuentascontables as cu WHERE cu.cuco_IdCuentaContable != 1 
			and  cu.cuco_IdCierre = $idDocumento;";
	$data = $con->consultar($query);
	if($data != NULL && $data != ""){
	if( $con->getNumeroFilasConsultadas($data) >0 ){
	while($res = $con->obnerFila($data)){
...
/*		$query = "SELECT cu.cuco_FechaCreacion as fecha, if(cu.cuco_IdTipoMovimiento = 1, 'Entrada', 'Salida') as tipo, 
		cu.cuco_Observacion as descripcion, (SELECT fo.forpa_Descripcion FROM fac_formas_pago as fo 
			WHERE fo.forpa_Id=cu.cuco_IdCuentaContable ) as cuenta, cu.cuco_Valor as valor
		FROM fac_cuentascontables as cu WHERE cu.cuco_IdCuentaContable = 1
		and cu.cuco_IdCierre = $idDocumento and cu.cuco_Observacion like '%Abono/Pago Factura a Credito%';";
*/
		...
		and cu.cuco_IdCierre = $idDocumento and cu.cuco_Observacion like '%Abono/Pago Factura a Credito%')
        UNION ALL
        (SELECT cu.cuco_FechaCreacion as fecha, if(cu.cuco_IdTipoMovimiento = 1, 'Entrada', 'Salida') as tipo, 
		cu.cuco_Observacion as descripcion, (SELECT fo.forpa_Descripcion FROM fac_formas_pago as fo 
			WHERE fo.forpa_Id=cu.cuco_IdCuentaContable ) as cuenta, CONCAT('-',cu.cuco_Valor) as valor
		FROM fac_cuentascontables as cu WHERE cu.cuco_IdCuentaContable = 1
		and cu.cuco_IdCierre = $idDocumento and cu.cuco_Observacion like '%Reintegro de Anulación - Abono Cuenta por Cobrar%')";
...
if($rowDocuMovimienEfectivo != null){
	...
	<table border="0.1" style="font-size:6px; text-align:center">
		<tr>
			<td style="width:160px;margin:auto;text-align:center">
				<strong>Abonos a Facturas a Credito - En Efectivo</strong>
			</td>
		</tr>
		<tr>
