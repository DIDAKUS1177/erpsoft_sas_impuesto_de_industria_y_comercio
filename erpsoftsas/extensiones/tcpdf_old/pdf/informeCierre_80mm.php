...
(SELECT subtipo.subtipa_Nombre from fac_sub_tipos_pagos as subtipo WHERE subtipo.subtipa_Id=pc.paca_IdSubTipoPago) as subTipoPago
		FROM fac_pagos_caja as pc where pc.paca_IdCaja  = $idDocumento and pc.paca_IdCierre IS NULL;";
$data = $con->consultar($query);
if($data != NULL && $data != ""){
if( $con->getNumeroFilasConsultadas($data) >0 ){ 
while($res = $con->obnerFila($data)){
...
*/
		...
		FROM fac_cuentascontables as cu WHERE cu.cuco_IdCuentaContable = 1 and cu.cuco_Cierre = 0 
			and cu.cuco_IdCierre IS NULL and cu.cuco_IdCaja = $idDocumento and cu.cuco_Observacion like '%Reintegro de Anulación - Abono Cuenta por Cobrar%')";
...
if(is_array($pagosCaja)){
	...
	<table border="0.1" style="font-size:6px; text-align:center">
		<tr>
			<td style="width:100px;margin:auto;text-align:rigth">
				<strong>TOTAL:</strong>
			</td>
			<td style="width:60px;margin:auto;text-align:left">
				$ $total_format
