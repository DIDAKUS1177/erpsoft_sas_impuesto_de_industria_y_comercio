...
$result = Builder::create()
    ->data('https://www.digitsoft.com.co') 
    ->size(50) // Tamaño del código QR
    ->margin(5) // Margen alrededor del código QR
    ->build();

// Convierte la imagen a base64
$imageData = base64_encode($result->getString());
$imageSrcqR = 'data:'.$result->getMimeType().';base64,'.$imageData;
...
//------------  Obtener Datos de la Factura y Cliente.
	$query = "SELECT fd.*, round(fd.doc_ValorNeto) as 'doc_ValorNeto', fd.doc_Prefijo as 'prefijo', fd.doc_Numero as 'numero', 
					...
					(select fdoo.doc_IdMesa from fac_documento_ordenes as fdoo where fd.doc_MesaActiva = fdoo.doc_Id ) as 'idMesa',
...
//------------  Obtener Datos del Domicilio del CLiente.
if($rowDocuClien[0]['doc_MesaActiva'] != 0){

	$idServicioMesa = $rowDocuClien[0]['doc_MesaActiva'];
	$query = "SELECT * FROM `fac_documento_ordenes` where doc_Id= $idServicioMesa";
	$data = $con->consultar($query);
	if($data != NULL && $data != ""){
		if( $con->getNumeroFilasConsultadas($data) >0 ){ 
			while($res = $con->obnerFila($data)){
				$rowMesaDocuClien[] = $res;
			}
		}else{ $rowMesaDocuClien = NULL;}
	}else{ $rowMesaDocuClien = NULL;}
...
//-------------  Obtener Prodcutos de la Facturación.
	...
	$query = "SELECT ipp.pro_Nombre as 'nombre', fdd.detDoc_Cantidad as 'cantidad', ium.uniM_Prefijo as 'unidad',
				...
				fpg.forpa_Descripcion as 'formaPago'
...
$valor_dado= number_format( $rowDocuClien[0]['doc_valor_dado'], 0,',','.');
$vueltas= number_format( $rowDocuClien[0]['doc_vueltas'], 0,',','.');
...
if(($rowDocuClien[0]['doc_MesaActiva'] != 0) and ($rowDocuClien[0]['idMesa'] == 100)){

foreach ($rowMesaDocuClien as $key => $item) {
...
<table style="text-align:center;font-size:6px; width:160px">
	<tr>
		<td style="width:160px">
			$texto
		</td>
	...
	<tr>
		<td>
			<img src="$imageSrcqR" style="width:80px">
		</td>
	</tr>
	<tr>
		<td style="width:160px">
			CUDE: <br> sjahdhsdasdsa54asd45sda454dda544a5dda54da54das4das4
		</td>
	</tr>
	<tr>
		<td style="width:160px">
...
//----------------------------------------------------------------------------------
// -------------- Información de ORDEN DE MESA FACTURADA  --------------------------
//----------------------------------------------------------------------------------
foreach ($rowDocuClien as $key => $item) {
$bloque2 = <<<EOF
	...
	<table style="font-size:6px; text-align:center">
		<tr>
			<td style="width:160px;margin:auto"></td>
		</tr>
		<tr>
			<td style="width:160px;margin:auto"></td>
		</tr>
...
// ------------- Información de Titulos de Productos --------------
$bloque4 = <<<EOF

<table border="0.1" style="font-size:6px; text-align:center">
	<tr>
		<td style="width:100px;margin:auto;text-align:left">
			Descripción
		</td>
		<td style="width:60px;margin:auto;text-align:left">
			Cantidad
