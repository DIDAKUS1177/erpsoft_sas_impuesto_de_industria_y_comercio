...
/**
 * Script de un solo uso para completar el catalogo conf_ciudades en
 * PRODUCCION con los 880 municipios que faltaban (solo tenia Bogota/
 * Boyaca/Cundinamarca, 240 filas -catalogo incompleto reportado por el
 * cliente como punto 4-). Fuente: DIVIPOLA/DANE via datos.gov.co, nombres
 * con tilde cruzados contra un dataset independiente (colombia-json).
 * Protegido con token de un solo uso. BORRAR este archivo (commit + pull en
 * Plesk) apenas se confirme que la migracion corrio bien.
 *
 * Idempotente: si un codigo DANE ya existe (p.ej. si esto se corre dos
 * veces, o si produccion ya tenia mas de lo esperado), se omite esa fila
...
if (($_GET['token'] ?? '') !== 'mig_municipios_2026_08_11_q9k3') {
...
$municipios = [
    ...
    ['05042','Santa Fe de Antioquia','Antioquia'],
    ...
    ['54239','Duranía','Norte de Santander'],
    ...
    ['54398','La Playa de Belén','Norte de Santander'],
    ...
    ['54660','Salazar de Las Palmas','Norte de Santander'],
    ...
    ['88001','San Andrés','San Andrés, Providencia y Santa Catalina'],
    ['88564','Providencia','San Andrés, Providencia y Santa Catalina'],
...
foreach ($municipios as $m) {
    list($dane, $nombre, $depto) = $m;

    $existe = $con->consultar("SELECT ciu_Id FROM conf_ciudades WHERE ciu_CodigoDane = ?", [$dane]);
    if ($existe && $con->obnerFila($existe)) {
        $omitidos++;
        continue;
    }

    $r = $con->consultar(
        "INSERT INTO conf_ciudades (ciu_CodigoDane, ciu_Nombre, ciu_Departamento, ciu_Estado, ciu_FechaCreacion, ciu_FechaActualizacion)
         VALUES (?, ?, ?, 1, GETDATE(), GETDATE())",
        [$dane, $nombre, $depto]
    );

    if ($r === false) {
        echo "ERROR insertando $dane $nombre\n";
        $errores++;
        continue;
    }
...
// Normaliza el casing de los 3 grupos que ya existian antes de esta
// migracion (venian con inconsistencias: "BOGOTA D.C." y "CUNDINAMARCA" en
// mayusculas mientras el resto usa Titulo Case). Solo cosmetico, no toca
// ninguna FK -ciu_Departamento es texto libre-.
$con->consultar("UPDATE conf_ciudades SET ciu_Departamento = 'Bogotá D.C.' WHERE ciu_Departamento = 'BOGOTÁ D.C.'");
$con->consultar("UPDATE conf_ciudades SET ciu_Departamento = 'Cundinamarca' WHERE ciu_Departamento = 'CUNDINAMARCA'");

$total = $con->obnerFila($con->consultar("SELECT COUNT(*) t FROM conf_ciudades"));

echo "Insertados: $insertados | Ya existian: $omitidos | Errores: $errores\n";
echo "Total final en conf_ciudades: {$total['t']}\n";
