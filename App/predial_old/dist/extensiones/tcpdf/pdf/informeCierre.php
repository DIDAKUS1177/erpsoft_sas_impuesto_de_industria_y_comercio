	...
	//------------  Obtener Datos de ultima y primera factura (PREFIJO Y NUMERO).
			$query = "(SELECT facdo.* FROM fac_tesoreria as ft 
			INNER JOIN fac_documento as facdo ON facdo.doc_Id = ft.teso_IdDocumento
					where ft.teso_Cierre = 0 and ft.teso_IdCaja = $idDocumento ORDER by facdo.doc_Id ASC limit 1)
					UNION ALL
					(SELECT facdo.* FROM fac_tesoreria as ft 
			INNER JOIN fac_documento as facdo ON facdo.doc_Id = ft.teso_IdDocumento
					where ft.teso_Cierre = 0 and ft.teso_IdCaja = $idDocumento ORDER by facdo.doc_Id desc limit 1)";
				$data = $con->consultar($query);
				if($data != NULL && $data != ""){
					if( $con->getNumeroFilasConsultadas($data) >0 ){ 
						while($res = $con->obnerFila($data)){
	...
	<table border="0.1" style="font-size:6px; text-align:center">
		...
		<tr>
			<td style="width:80px;margin:auto;text-align:left">
				DE $item[0][doc_Prefijo]  $item[0][doc_Numero]
			</td>
			<td style="width:80px;margin:auto;text-align:left">
				AL $item[1][doc_Prefijo]  $item[1][doc_Numero]
			</td>
		</tr>
