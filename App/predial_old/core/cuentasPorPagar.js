...
class Nota {
    ...
    draw_table_documents(arrFilter) {
        ...
        for (let not of arrFilter) {
            ...
            // Se filtra solo las entradas con proveedores
            if(not.kar_IdProveedor != null ){
            if (not.kar_EstadoPago == 0) {
                var icono = "dw dw-ban";
                ...
                if (not.kar_TipoPago == 1) {
                    var formaPago = "Contado";
                } else if(not.kar_TipoPago == 2) {
                    var formaPago = "Credito";
                }else{
                    var formaPago = "No Aplica";
                }

                var totalNota = Math.trunc(not.strValorTotalCuenta);
                
                if (not.kar_EstadoPago == 0) {
                    var estadoPago = "No Saldada";
                    var btncargar = '<button type="button" class="btn btn-social-icon btn-warning " data-toggle="tooltip" title="Abonar/Saldar Cuenta" style="margin-right:5px" onclick="javascript:nota.getAbonoCuentaById('+not.kar_Id+','+totalNota+')"><i class="dw dw-edit2"></i></button>';
                }else if(not.kar_EstadoPago == 1) {
                ...
                $('#bodyNotasRegistradas').append(
                    ...
                    '</td>' +
                    '<td>' +
                    not.strNombreProveedor +
                    '</td>' +
                    '<td>' +
                    not.kar_Observaciones +
                    '</td>' +
                    '<td>' +
                    '$ '+ Number(parseInt(not.strValorTotalCuenta).toFixed(0)).toLocaleString('es-CO') +
                    ...
                    '</td>' +
                    '<td>' +
                    '$ '+ Number(parseInt(not.strValorTotalCuenta) - parseInt(totalAbonos)).toLocaleString('es-CO') +
                    '</td>' +
                    '<td>' +
                    estadoPago +
                    '</td>' +
                    ...
                    '<td align="center">' +
                    btncargar +
                    /* '<button type="button" class="btn btn-social-icon btn-warning " data-toggle="tooltip" title="Abonar/Saldar Cuenta" style="margin-right:5px" onclick="javascript:nota.getAbonoCuentaById('+not.kar_Id+','+totalNota+')">'+
                    ...
                    '</button>' +
                    '<a href="../extensiones/tcpdf/pdf/soporteProveedor.php?codigo=' + not.kar_Id + '" target="_blank" title="Descargar Soporte" class="btn btn-info btn-pill"><span class="ti-arrow-down"></span></a>' +
                    '</td>' +
     ...
     */
        async getAbonoCuentaById(id, totalNota) {

            var permiso = await _permisos.getPermisos(idRol, 2392);
            ...
            if (permiso.ok != 1) {
                ...
                $("#formCrearAbonosCuentas").attr('action', 'javascript:nota.postAbonosCuentas(' + id + ','+totalNota+')');
     ...
     /**
     * postAbonosCuentas: Método para insertar abonos cuentas
     */
      postAbonosCuentas(id,totalNota) {
        var Kar_Valor = $("#Kar_Valor").val();
        var kar_Cuentas = $("#kar_Cuentas").val();
        var id_caja = sessionStorage.getItem('id_caja');

        $.ajax({
            url: '../business/controller/class.cuentasPorPagar.php',
            data: { funcion: 1, id: id, Kar_Valor: Kar_Valor, kar_Cuentas: kar_Cuentas, 
                    totalNota: totalNota, doc_IdSerieCaja: id_caja},
            dataType: "json",
            type: "POST",
...
//        var permiso = await _permisos.getPermisos(idRol, 521);
    ...
    NotaActivo() {
        ...
        $("#SubMenuCuentasPorPagar").addClass('active');
    }
