...
class Nota {
     ...
     */
    async crearNota() {
        var permiso = await _permisos.getPermisos(idRol, 27107);
        ...
        if (permiso.ok != 1) {
            ...
            $("#formCrearNota").attr('action', 'javascript:nota.postNota()');
             ...
             // Obtenemos el ID de la preforma a editar.
             var doc_idOrdenEditar = $("#idOrdenEditar").val();
             var doc_idMesa = $("#idMesa").val();

            await nota.getProducto();
            //await nota.getBodega();
            await nota.getClientes(doc_idMesa);
            await nota.cargarPreciosOrdenEditar(doc_idOrdenEditar); 
            //await nota.formasPago();
            await nota.cargarFacturacion(doc_idOrdenEditar);
    ...
    getNotas() {
        ...
        if(idRol == 1){
            ...
            $.ajax({
                url: '../business/controller/class.facturaOrdenes.php',
                data: { funcion: 3, doc_IdVendedor: idVendedor, doc_Estado: 1},
                //data: { funcion: 3 },
                dataType: "json",
                type: "POST",
                success: function(arr) {
    ...
    postNota() {
        ...
        if (detallesNota.length < 1) {
            ...
            var idOrdenEditar = $("#idOrdenEditar").val();
                                    ...
                                    var nom_factura = sessionStorage.getItem('nom_tipoFactura');
                                        ...
                                        if (nom_factura == 1) {
                                            ...
                                            //  CONFIGURACIÓN PARA CONSUMO DESDE EL COMPUTADOR.
                                                ...
                                                $('.modal-headerr').append('<p class="heading lead" style="font-size:30px;"><strong>ORDEN EDITADA</strong> </strong></p>');
    ...
    NotaActivo() {
        ...
        $("#SubMenuCrearOrdenes").addClass('active');
    }
            ...
            success: function(arr) {
                ...
                if (arr.ok == 1) {
                    $.each(arr.datos, function(k, v) {
                        ...
                        if(v['seemma_Id']  ==  id){
                                $("#doc_IdMesa").append('<option selected="selected"  value="' + v['seemma_Id'] + '">'+ v['seemma_Nombre'] + ' </option>');
                        }else{
                            if(v['estado'] == 0){
    ...
    */
     ...
     */
    cargarFacturacion(doc_idOrdenEditar) {
        ...
        $.ajax({
            url: '../business/controller/class.facturaOrdenes.php',
            data: { funcion: 3, id: doc_idOrdenEditar},
            dataType: "json",
            type: "POST",
            success: function(arr) {
                console.log('arr ', arr);
    ...
    /**
     * cargarPreciosOrdenEditar: Método para cargar los precios del producto
     */
      cargarPreciosOrdenEditar(doc_idOrdenEditar) {

        $.ajax({
            url: '../business/controller/class.facturaOrdenes.php',
            data: { funcion: 12, idOrdenEditar: doc_idOrdenEditar },
            dataType: "json",
            type: "POST",
            success: function (arr) {
                console.log('productos ', arr);
                $.each(arr.datos, function (k, v) {

                    var detDoc_IdProducto = v['detDoc_IdProducto'];
                    var detDoc_Cantidad = v['detDoc_Cantidad'];

                    var detDoc_Cantidad = v['detDoc_Cantidad'];
                    var detDoc_ValorUnitario = v['detDoc_ValorUnitario'];
                    var detDoc_ValorTotal = v['detDoc_ValorTotal'];
                    var strNombreProducto = v['strNombreProducto'];

                    var detkar_Descuento = $("#detkar_Descuento").val();
                    nota.cargarStockEditar(detDoc_IdProducto);

                    $.ajax({
                        url: '../business/controller/class.preciosVenta.php',
                        data: { funcion: 3, preVen_IdProducto: detDoc_IdProducto },
                        dataType: "json",
                        type: "POST",
                        success: function (arr) {
                            console.log('arASTA ACAr ', arr);
                            $("#detkar_Costo").empty();
                            ...
                            //$("#detkar_Costo").addClass('custom-select2');                
                            nota.agregarDetalleEditarPrforma(detDoc_IdProducto, detDoc_Cantidad, detDoc_ValorUnitario, detDoc_ValorTotal,strNombreProducto);
      ...
      /**
     * cargarStockEditar Stock: Método para cargar el stock del producto
     */
      cargarStockEditar(detkar_IdProducto) {
    ...
    agregarDetalleEditarPrforma(detDoc_IdProducto, detDoc_Cantidad, detDoc_ValorUnitario, detDoc_ValorTotal, strNombreProducto ) {

        var stockActivo = $("#detkar_Stock").val();
        var pro = detDoc_IdProducto;
        var cant = detDoc_Cantidad;
        var cost = parseInt(detDoc_ValorTotal);
        var costTex = parseInt(detDoc_ValorTotal);
        var costUni = parseInt(detDoc_ValorUnitario);
        var bod = $("#detkar_IdBodega").val();
        ...
        if (pro != "" && cant != "" && cost != "" && bod != "") {
            ...
            var namePro = strNombreProducto;
    
            var nameBod = $('#detkar_IdBodega option:selected').text();

            detallesNota.push({
                "detkar_Bruto": bruto, "detkar_Impuesto": impuesto,
                "detkar_IdProducto": pro, "detkar_Cantidad": cant,
                "detkar_Costo": cost, "detkar_CostoUnitario": costUni,
                "detkar_CostoText": costTex, "detkar_IdBodega": bod,
                "nomProducto": namePro, "nomBodega": nameBod, "idImpuesto": impu
