    ...
    async pintarTable() {
        ...
        if (detallesNota.length > 0) {
            ...
            for (let d of detallesNota) {
                  ...
                  var permiso = await _permisos.getPermisos(idRol, 6104);
...
//                console.log('array detalle ', detallesNota);
    ...
    async eliminarDetalle(posicion) {
        ...
        var permiso = await _permisos.getPermisos(idRol, 6105);
    ...
    postNota() {
        ...
        if (detallesNota.length < 1) {
            ...
            var idMesaActiva = $("#idMesaActiva").val();
...
//  CONFIGURACIÓN PARA CONSUMO DESDE EL COMPUTADOR.
                    ...
                    $('.modal-headerr').append('<a type="button" id="nodescargar" onclick="nota.descargarRem(2)" class="btn btn-outline-info waves-effect" data-dismiss="modal">Cerrar<i class="mi mi-ButtonX"></i></a><p>');
                    $('.modal-headerr').append('<p class="heading lead" style="font-size:30px;"><strong>FACTURA CREADA</strong> / Vueltas <strong>$'+ Number(vueltas.toFixed(0)).toLocaleString('es-CO') +'</strong></p>');
...
//  CONFIGURACIÓN PARA CONSUMO DESDE EL CELULAR, TABLET.
    ...
    NotaActivo() {
        ...
        $("#SubMenuRemision").addClass('active');
    }
    ...
    */
     ...
     * getClientes: Método para cargar los precios del producto
     */
            cargarPreciosOrden(idProducto, idCantidad, valor) {

                var detkar_IdProducto = idProducto;
                
                console.log('precios: ', detkar_IdProducto);
                $("#detkar_TipoProduc").val(2);
                $("#detkar_Cantidad").val(idCantidad);
                $("#detkar_Costo").empty();
                $("#detkar_Costo").val(valor);    
                $("#impuesto").val(0);
    
            var stockActivo = 100000000;
            var pro = idProducto;
            var cant = idCantidad;
            var cost = valor * idCantidad;
    
             var costTex = valor * idCantidad;
            ...
            console.log('cantidad ', cant,'-stock-',stockActivo);
                ...
                if (pro != "" && cant != "" && cost != "" && bod != "") {
                    ...
                    detallesNota.push({ "detkar_Bruto": bruto, "detkar_Impuesto":0,
                                        "detkar_IdProducto": pro, "detkar_Cantidad": cant, 
                                        "detkar_Costo": cost, "detkar_CostoUnitario": costUni,
                                        "detkar_CostoText": costTex, "detkar_IdBodega": bod,
                                        "nomProducto": namePro, "nomBodega": nameBod, "idImpuesto": 4 });
...
*/
     ...
     */
     descargarRem(id) {
        ...
        //window.print();
        //window.close();
        window.open('../dist/remision.php','_self');        
    }
                ...
                if (arr.ok == 1) {
                    $.each(arr.datos, function(k, v) {  
                        if(v['estado'] == 1){                         
                            ...
                            $('#divMesas').append('<button class="btn btn-primary" style="margin-right:20px; margin-top:20px" onclick="nota.cargarOrden('+v['seemma_Id']+')" type="button">' + v['seemma_Nombre'] + ' </button>');
                        }
                    });
                }                
            },
    ...
    cargarOrden(idMesa) {
        ...
        $.ajax({
            ...
            success: function(arr) {
                ...
                if (arr.ok == 1) {
                    $.each(arr.datos, function(k, v) {
                        var detkar_IdProducto = $("#detkar_IdProducto").val(v['detDoc_IdProducto']);
                        var detkar_Cantidad = $("#detkar_Cantidad").val(v['detkar_Cantidad']);
                        nota.cargarPreciosOrden(v['detDoc_IdProducto'], v['detDoc_Cantidad'],  v['valor']);
                        $("#idMesaActiva").val(v['idServicioMesa']); 
                        //nota.cargarPrecios();
                    });
                    nota.pintarTable();
                }                
