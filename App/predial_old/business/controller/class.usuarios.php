...
include_once SERVER . '/business/DAO/DAO_UsuarioCaja.php';
        ...
        try {
            ...
            switch ($_obj->_funcion) {
                ...
                case 4:
                    ...
                    break; 
                case 5:
                    $respuesta = $_obj->_AgregarUsuariCaja();
                    break; 
            }
            $con->commit();
            //$_obj->cabeceras();
...
/*
                // Crear Asociación USUARIO CAJA
                if($_POST['sede'] =! 0){
                    $usuario = $this->_agregarUsuarioCaja($_POST['sede'], $_POST['caja'], $id); 
                }
*/                
                $_objlogs = new logs();
                //$_objlogs->_insertLogs($id,$_SESSION['id_usuario'],2,7);
                $this->_ok = 1;
    ...
    protected function _editarUsuario() {
        ...
        if($nomduplicado == 1){
            ...
            if(!$_objUsuario->guardar()){
                ...
                // Crear Asociación USUARIO CAJA
                $usuario = $this->_editarUsuarioCaja($_POST['IdUsuarioCaja'], $_POST['sede'], $_POST['caja'], $_POST['id'] ); 

                $_objlogs = new logs();
                $_objlogs->_insertLogs($id,$_SESSION['id_usuario'],2,8);
                $this->_ok = 1;
                $this->_mensaje = "Datos ingresados correctamente";
            }
            $return= $_objUsuario->guardar();
    ...
    **/  
    protected function _inactivarUsuarios() {
        ...
        if(!$_objUsuario->guardar()){
            ...
            $id = $_objUsuario->get_usu_Id();
            $_objlogs = new logs();
            $_objlogs->_insertLogs($id,$_SESSION['id_usuario'],2,9);
            $this->_ok = 1;
    ...
    /**
    *** Realiza el proceso de Agregar Usuario a Cajas.
    **/  
    protected function _agregarUsuarioCaja($IdSede, $IdCaja, $IdVendedor) {
        
        $_objUsuario = new \predial\DAO_UsuarioCaja();
        $_objUsuario->set_usuca_IdSede($IdSede);
        $_objUsuario->set_usuca_IdCaja($IdCaja);
        $_objUsuario->set_usuca_IdVendedor($IdVendedor); 
    ...
    /**
    *** Realiza el proceso de Editar usuario a Cajas.
    **/  
    protected function _editarUsuarioCaja($Id, $IdSede, $IdCaja, $IdVendedor) {
        
        $_objUsuario = new \predial\DAO_UsuarioCaja();
        $_objUsuario->set_usuca_Id($Id);
        $_objUsuario->set_usuca_IdSede($IdSede);
        $_objUsuario->set_usuca_IdCaja($IdCaja);
        $_objUsuario->set_usuca_IdVendedor($IdVendedor); 
