...
include_once SERVER . '/business/DAO/DAO_ProductoComposicion.php';
include_once SERVER . '/business/class.sessions.php';
include_once SERVER .'/business/controller/class.logs.php';

class ControladorProductoComposicion extends \predial\Cabecera {
            ...
            switch ($_obj->_funcion) {
                ...
                case 2:
                   // $respuesta = $_obj->_editarBodega();
                    break;
                case 3:
                    $respuesta = $_obj->_consultarProductoComposicion();
                    break; 
                case 4:
    ...
    **/ 
    ...
    /**
    *** Realiza el proceso de Consultar Producto Conposicion.
    **/ 
    private function _consultarProductoComposicion() {
       
        $_objRol = new \predial\DAO_ProductoComposicion();
        if(isset($_POST['id'])){
            if (!empty($_POST['id']) || $_POST['id'] != NULL ) {
                $_objRol->set_proco_Id($_POST['id']);
            }    
        }

        if(isset($_POST['idProducto'])){
            if (!empty($_POST['idProducto']) || $_POST['idProducto'] != NULL ) {
                $_objRol->set_proco_IdProducto($_POST['idProducto']);
            }    
        }

        if(isset($_POST['idInsumo'])){
            if (!empty($_POST['idInsumo']) || $_POST['idInsumo'] != NULL ) {
                $_objRol->set_proco_IdInsumo($_POST['idInsumo']);
            }    
        }
        ...
        if(is_array($arrRol) && count($arrRol)){
            ...
            $this->_ok = 1;
            $this->_mensaje = "Producto Composicion listadas con exito";
        }else{
            $R=$_objRol;
            $this->_ok = 0;
            $this->_mensaje = "No existen Producto Composicion";            
        }
        return $R;
    }  
}

class ProductoComposicionException extends \Exception{}

    \predial\ControladorProductoComposicion::run();
