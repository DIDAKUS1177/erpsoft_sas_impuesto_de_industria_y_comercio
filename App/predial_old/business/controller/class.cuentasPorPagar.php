...
include_once SERVER . '/business/DAO/DAO_CuentasPorPagar.php';
...
class ControladorCuentasPorPagar extends \predial\Cabecera {
    ...
    public static function run() {
        ...
        try {
            ...
            switch ($_obj->_funcion) {
                case 1:
                    $respuesta = $_obj->_agregarCuentasPorPagar();
                    break;
//                case 2:
//                    $respuesta = $_obj->_editarCuentasContables();
//                    break;
                case 3:
                    $respuesta = $_obj->_consultarCuentasPorPagar();
                    break; 
//                case 4:
...
//                    break;
    ...
    */ 
    protected function _agregarCuentasPorPagar() {
        
        $_objCuentasContables = new \predial\DAO_CuentasPorPagar();
        $_objCuentasContables->set_cupa_IdNota($_POST['id']);
        $_objCuentasContables->set_cupa_IdCuentaContable($_POST['kar_Cuentas']);
        $_objCuentasContables->set_cupa_Valor($_POST['Kar_Valor']);

        if(!$_objCuentasContables->guardar()){
            $this->_ok = 0;
            $this->_mensaje = $_objCuentasContables->getMysqlError();
        }else{
            $id = $_objCuentasContables->get_cupa_Id();
            
            // INSERTAR movimeinto en cuenta contable
            $observacion= "PAGO PROVEEDOR - Numero Documento #  ".$_POST['id'];
            $this->_agregarCuentasContables($_POST['doc_IdSerieCaja'],2,$_POST['kar_Cuentas'], 
            ...
            if($totalAbonos >= $_POST['totalNota']){
                $this->_saldarCuenta($_POST['id']);
            }
            
            $this->_ok = 1;
            $this->_mensaje = "Abono cuentas por pagar ingresados correctamente";
        }
    ...
    */ 
    protected function _saldarCuenta($id) {
        ...
        $_objMarca = new \predial\DAO_Nota();
        $_objMarca->set_kar_Id($id);
        $_objMarca->set_kar_EstadoPago(1);

        if(!$_objMarca->guardar()){
            $this->_ok = 0;
            $this->_mensaje = $_objMarca->getMysqlError();
        }else{
            $id = $_objMarca->get_kar_Id();
            //$_objlogs = new logs();
    ...
    */ 
    protected function _agregarCuentasContables($idCaja, $tipo, $idCuentaContable, $idDocumento, $valor, $observacion) {
        ...
        $_objCuentasContables->set_cuco_IdTipoSalida(2);
    ...
    */ 
    private function _consultarCuentasPorPagar() {
    ...
    protected function _consultarConsolidado($id) {
        ...
        $query = "SELECT SUM(cu.cupa_Valor) as tot 
            FROM fac_cuentas_por_pagar as cu where cu.cupa_IdNota = $id";
...
class CuentasPorPagarException extends \Exception{}

    \predial\ControladorCuentasPorPagar::run();
