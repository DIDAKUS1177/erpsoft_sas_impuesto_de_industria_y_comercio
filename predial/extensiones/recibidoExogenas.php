    ...
    public function traerImpresionFactura() {
        ...
        $query = "
            SELECT 
                e.exo_Id,
                e.exo_Anio,
                e.exo_IdTipoDocumento,
                e.exo_FechaCreacion,
                u.usu_Nombre,
                u.usu_NumeroDocumento
            FROM exogenas e
            INNER JOIN conf_usuario u ON u.usu_id = e.exo_IdUsuario
            WHERE e.exo_Id = $idExogena
        ";
        ...
        if ($con->getNumeroFilasConsultadas($data) > 0) {
            ...
            die("No se encontraron datos para el idExogena.");
        }

        // 🔴 DATOS
        $cedula  = $row['usu_NumeroDocumento'];
        $nombre  = $row['usu_Nombre'];
        $formato = $row['exo_IdTipoDocumento'];
        $anioBD  = $row['exo_Anio'];
        $fechaBD = $row['exo_FechaCreacion'];

        // 🔴 ACUSE DINÁMICO
        $consecutivo = str_pad($idExogena, 4, "0", STR_PAD_LEFT);
        $acuse = $anioBD . $consecutivo;
        ...
        $fechaExpedido = date('j \d\e F \d\e Y');
        ...
        $html  = '
            ...
            <table cellpadding="4" cellspacing="0" border="0">
                <tr><td width="35%"><strong>Acuse de recibido:</strong></td><td> ' . $acuse . '</td></tr>
            ...
            <p style="text-align:right;">Expedido el <em>' . $fechaExpedido . '</em></p>
